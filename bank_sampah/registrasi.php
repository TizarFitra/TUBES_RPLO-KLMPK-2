...
if (isset($_POST['register'])) {
    ...
    $stmt = $conn->prepare("SELECT id FROM nasabah WHERE username = ?");
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO nasabah (nama, alamat, telepon, username, password, saldo) VALUES (?, ?, ?, ?, ?, 0)");
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $stmt->bind_param("sssss", $nama, $alamat, $telepon, $username, $hashed_password);
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = "Registrasi gagal: " . $stmt->error;
        }
        $stmt->close();
    }
}
...
<body class="bg-gradient-to-br from-green-500 to-green-700 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 space-y-6 leaf-pattern">
        <div class="text-center space-y-2">
            ...
            <h2 class="text-3xl font-bold text-gray-800">Registrasi Nasabah</h2>
        ...
        <form method="POST" action="" class="space-y-5">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="alamat" required minlength="5" maxlength="255"
                    class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-200 bg-gray-50"
                    placeholder="Masukkan alamat lengkap" value="<?php echo isset($alamat) ? htmlspecialchars($alamat) : ''; ?>">
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="tel" name="telepon" required pattern="[0-9]{10,13}"
                    class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-200 bg-gray-50"
                    placeholder="Contoh: 08123456789" value="<?php echo isset($telepon) ? htmlspecialchars($telepon) : ''; ?>">
            </div>
        ...
        <p class="text-center text-gray-600">
            Sudah punya akun? 
            <a href="login.php" class="text-green-600 hover:text-green-700 font-semibold transition duration-200">
                Login di sini
            </a>
        </p>
    </div>
